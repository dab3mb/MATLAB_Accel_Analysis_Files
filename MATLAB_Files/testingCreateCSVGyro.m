...
fingerNumber = "testing16";
fingerOrder  = ["p","r","m","i"];
data = importfile("RecordedDataGyro-" + "test" +  ".csv",[1,inf]);
x = data.x(15:end-15);
y = data.y(15:end-15);
z = data.z(15:end-15);
...
% Use x data to find 'events' or 'taps'
[xTimeIndex,xData,sampleX]= getTaps(filteredDataX, averageFilteredX, 28, time, 2, "_gyro_x", fingerOrder);                

% Use 'event' or 'tap' times from x0 (xTimeIndex) to get the data for z & x
sampleZ = getZYFromX(filteredDataZ, xTimeIndex, time, "_gyro_z");
sampleY = getZYFromX(filteredDataY, xTimeIndex, time, "_gyro_y");


% Create final Model
Final_Model_GYRO_TESTING16 = [sampleZ(:,1:end), sampleY(:,1:end), sampleX(:, 1:end)];
Final_Model_GYRO_TESTING16
...
save(fileName , 'Final_Model_GYRO_TESTING16')
...
function [indexList, valuesList, samples] = getTaps(direction, averageDirection, chunkSize, time, significance, fingerAndDirection, justFinger)
    ...
    size(samples)
    samples = addvars(samples, ["p","p","p","p","r","r","r","r","m","m","m","m","i","i","i","i"]');
